...
- 直接写
  ...
  - 赋值在后：新，vue2 下也是新的，数据劫持是先做的，赋值也是先触发的，这个时候更新渲染watcher还没有执行（猜测vue3同理）。
- setTimeOut 
  - 赋值在前：新
...
- Promise.resolve
  - 赋值在前：新
  - 赋值在后：更新 effect，会赋值到 currentFlushPromise，在执行nextTick时，返回 currentFlushPromise，即在更新 effect 后执行。vue2 是旧的没有 currentFlushPromise
- 用户行为事件回调执行
  - 赋值在前：新
  - 赋值在后：旧，同（2）
