...
- 直接写
  ...
  - 赋值在后：初次更新 effect ，即更新 effect，会赋值到 currentFlushPromise，在执行nextTick时，返回 currentFlushPromise，即在更新 effect 后执行 **（1）**
- setTimeOut 
  - 赋值在前：新
  - 赋值在后：此时，currentFlushPromise 为 null，会生成行的 Promise.resolve() 来生成新的微任务，在 vue 的任务队列中比更新 effect 靠前，会先自行，所以是旧的 **（2）**
- Promise.resolve
  - 赋值在前：新
  - 赋值在后：同（1）
- 用户行为事件回调执行
...
> 实际使用还是看更新 effect 和 nextTick 的顺序，如果更新 effect 在前，nextTick 在后，则获取的是最新的，反之，则获取的是旧的。至于为什么会出现（1）的情况，是因为vue 的更新是在一个队列中，执行完了会生成一个 Promise.resolve() 来置空 currentFlushPromise。如果此时直接写/直接生成异步，currentFlushPromise 还没有置空，所以是在更新 effect 的回调中执行，所以是新的。
[更新 effect, nextTick, Promise.resolve() 置空]
