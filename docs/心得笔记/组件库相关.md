...
// 有开启才修改，控制影响范围 // [!code ++]
if (config.value.openMemory) { // [!code ++]
    syncMenuState_plus(values, forced); // [!code ++]
} else { // [!code ++]
    const nodes = unique(
        values.map((val) =>
...
const syncMenuState_plus = (values, reserveExpandingState = true) => { // [!code ++]
    ...
    const oldNodes = checkedNodes.value; // [!code ++]

    // 多重循环优化为单重循环 [!code ++]
    let newNodes = []; // [!code ++]
    for (let index = 0; index < values.length; index++) { // [!code ++]
        const node = store.getLeafNodeByValue(values[index].toString()); // [!code ++]
        if (!!node && (checkStrictly || node.isLeaf)) { // [!code ++]
            newNodes.push(node); // [!code ++]
            node.doCheck(true); // [!code ++]
        } // [!code ++]
    } // [!code ++]

    const oldExpandingNode = // [!code ++]
        store == null ? void 0 : store.getSameNode(expandingNode.value); // [!code ++]
    const newExpandingNode = // [!code ++]
        (reserveExpandingState && oldExpandingNode) || newNodes[0]; // [!code ++]
    if (newExpandingNode) { // [!code ++]
        newExpandingNode.pathNodes.forEach((node) => expandNode(node, true)); // [!code ++]
    } else { // [!code ++]
        expandingNode.value = null; // [!code ++]
    } // [!code ++]
    oldNodes.forEach((node) => node.doCheck(false)); // [!code ++]
    checkedNodes.value = newNodes; // [!code ++]
    nextTick(scrollToExpandingNode); // [!code ++]
}; // [!code ++]
```

```js
// node_modules/element-plus/es/components/cascader-panel/src/store.mjs
// 增加 leafMap 参数，用于存储叶子节点
...
class Store {
    ...
    // 直接查询避免循环 [!code ++]
    getLeafNodeByValue(value) { // [!code ++]
        return this.leafMap.get(value); // [!code ++]
    } // [!code ++]
}
```
